...
export const useAuthStore = create((set, get) => ({
  user: null, profile: null, tenant: null, loading: true, error: null,
  ...
  loadProfile: async (user) => {
    ...
    try {
      const { data: profile, error: ep } = await supabase.from("profiles").select("*").eq("id", user.id).single()
      if (ep) throw ep
      let tenant = null
  ...
  signOut: async () => { await supabase.auth.signOut(); set({ user: null, profile: null, tenant: null }) },
